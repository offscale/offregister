...
class OffAnsible(OffregisterBaseDriver):
    ...
    def __init__(self, env, node, node_name, dns_name, **kwargs):
        ...
        self.driver_node_env = {
            ...
            and k not in ("count", "index")
            and not isinstance(getattr(env, k), property)
        }
        ...
        self.passwords = update_d(
            dict(vault_pass="secret"),
            ansible_settings.get("passwords"),
            conn_pass=self.env_namedtuple.password
            if self.env_namedtuple.password
            else None,
        )
    ...
    def prepare_cluster_obj(self, cluster, res):
        ...
        if self.env.key_filename is None:
            extra_vars.update(
                {"ansible_ssh_private_key_file": self.env_namedtuple.key_filename}
            )
        if (
            self.env.ssh_config is not None
            and "StrictHostKeyChecking" in self.env.ssh_config
        ):
            host_key_checking = (
                self.env_namedtuple.ssh_config["StrictHostKeyChecking"] == "yes"
            )
            extra_vars.update(
                {
    ...
    def run_tasks(
        ...
        try:
            ...
            if self.dns_name not in res:
                res[self.dns_name] = OrderedDict(
                    {cluster_path: OrderedDict({t: result})}
                )
...
class ResultCallback(CallbackModule_default):
    ...
    def pp_lines(log, result, key, op=None, op_args=None):
        it_consumes(
            list(
                map(
                    lambda l: log.info(
                        "{host} - {l}".format(host=result._host.name, l=l)
                    ),
                    result._result[key]
                    if op is None
                    else getattr(result._result[key], op)(
    ...
    def error_result(location_or_log, result):
        log = (
            get_logger("ansible::results_callback::{}".format(location_or_log))
            if isinstance(location_or_log, str)
            else location_or_log
        )
